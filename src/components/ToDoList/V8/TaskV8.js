...
import {deleteTask, updateTask} from "./ServicesV8";
import {clearCopletedCreator8, deleteTaskCreator8, updateTaskCreator8} from "./Redux8/DotoList-Actions";



export default class TaskV8 extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            editMode: false,
    ...
    deleteTask2 ()  {
        deleteTask(53336, this.props.task.id).then(data => {
            //this.parentDeleteCalback(this.props.task.id);
            this.props.store.dispatch(deleteTaskCreator8(this.props.task.id));
            this.setState({
    ...
    // })
        ...
        updateTask(53336, task.id ,null ,task.isDone).then(data => {
            ...
            this.props.store.dispatch(updateTaskCreator8(task));
            this.setState({
